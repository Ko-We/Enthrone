...
public class ShopTemplateMgr : MUIBase<ShopTemplate>
{
    string TextResource;
    bool flagDataLoad;
    ...
    public void OnDataLoad(string _textResource)
    {
        ...
        if (Data != "")
        {
            ...
            for (int nCnt = 1; nCnt < list.Length; nCnt++)
            {
                ...
                {
                    ShopTemplate pTemplate = new ShopTemplate(listDetail);
    ...
    public ShopTemplate GetTemplate(int _Index)
    {
        List<ShopTemplate> listTemplate = GetTList();
