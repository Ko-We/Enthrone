...
public class GachaTemplateMgr : MUIBase<GachaTemplate>
{
    string TextResource;
    bool flagDataLoad;

    public GachaTemplateMgr()
    {
        flagDataLoad = false;
    }
    ...
    public void OnDataLoad(string _textResource)
    {
        ...
        if (Data != "")
        {
            ...
            for (int nCnt = 1; nCnt < list.Length; nCnt++)
            {
                ...
                {
                    GachaTemplate pTemplate = new GachaTemplate(listDetail);
    ...
    public GachaTemplate GetTemplate(int _Index)
    {
        List<GachaTemplate> listTemplate = GetTList();
