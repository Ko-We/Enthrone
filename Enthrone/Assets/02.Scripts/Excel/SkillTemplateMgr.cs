...
public class SkillTemplateMgr : MUIBase<SkillTemplate>
{
    string TextResource;
    bool flagDataLoad;

    public SkillTemplateMgr()
    {
        flagDataLoad = false;
    }
    ...
    public void OnDataLoad(string _textResource)
    {
        ...
        if (Data != "")
        {
            ...
            for (int nCnt = 1; nCnt < list.Length; nCnt++)
            {
                ...
                {
                    SkillTemplate pTemplate = new SkillTemplate(listDetail);
    ...
    public SkillTemplate GetTemplate(int _Index)
    {
        List<SkillTemplate> listTemplate = GetTList();
