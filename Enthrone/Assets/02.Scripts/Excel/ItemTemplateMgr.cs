...
public class ItemTemplateMgr : MUIBase<ItemTemplate>
{
    string TextResource;
    bool flagDataLoad;

    public ItemTemplateMgr()
    {
        flagDataLoad = false;
    }
    ...
    public void OnDataLoad(string _textResource)
    {
        ...
        if (Data != "")
        {
            ...
            for (int nCnt = 1; nCnt < list.Length; nCnt++)
            {
                ...
                {
                    ItemTemplate pTemplate = new ItemTemplate(listDetail);
    ...
    public ItemTemplate GetTemplate(int _Index)
    {
        List<ItemTemplate> listTemplate = GetTList();
