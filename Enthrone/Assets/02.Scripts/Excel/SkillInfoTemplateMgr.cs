...
public class SkillInfoTemplateMgr : MUIBase<SkillInfoTemplate> {
    string TextResource;
    bool flagDataLoad;

    public SkillInfoTemplateMgr()
    {
        flagDataLoad = false;
    }
    ...
    public void OnDataLoad(string _textResource)
    {
        ...
        if (Data != "")
        {
            ...
            for (int nCnt = 1; nCnt < list.Length; nCnt++)
            {
                ...
                {
                    SkillInfoTemplate pTemplate = new SkillInfoTemplate(listDetail);
    ...
    public SkillInfoTemplate GetTemplate(int _Index)
    {
        List<SkillInfoTemplate> listTemplate = GetTList();
