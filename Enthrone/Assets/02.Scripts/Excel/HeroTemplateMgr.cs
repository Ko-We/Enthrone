...
public class HeroTemplateMgr : MUIBase<HeroTemplate>
{
    string TextResource;
    bool flagDataLoad;

    public HeroTemplateMgr()
    {
        flagDataLoad = false;
    }
    ...
    public void OnDataLoad(string _textResource)
    {
        ...
        if (Data != "")
        {
            ...
            for (int nCnt = 1; nCnt < list.Length; nCnt++)
            {
                ...
                {
                    HeroTemplate pTemplate = new HeroTemplate(listDetail);
    ...
    public HeroTemplate GetTemplate(int _Index)
    {
        List<HeroTemplate> listTemplate = GetTList();
