...
public class CashShopTemplateMgr : MUIBase<CashShopTemplate>
{
    string TextResource;
    bool flagDataLoad;

    public CashShopTemplateMgr()
    {
        flagDataLoad = false;
    }
    ...
    public void OnDataLoad(string _textResource)
    {
        ...
        if (Data != "")
        {
            ...
            for (int nCnt = 1; nCnt < list.Length; nCnt++)
            {
                ...
                {
                    CashShopTemplate pTemplate = new CashShopTemplate(listDetail);
    ...
    public CashShopTemplate GetTemplate(int _Index)
    {
        List<CashShopTemplate> listTemplate = GetTList();
